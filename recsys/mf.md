...
$k \ll \min(m, n)$, $k$的大小决定了隐向量表达能力的强弱，$k$的取值越小，隐向量包含的信息就越少，相对来讲泛化能力就越高；反之$k$取值越大，隐向量的表达能力越强，但泛化能力相对降低。
...
优点：
- 泛化能力强，解决了数据稀疏的问题
- 空间复杂度低。不在需要用户相似度矩阵或者物品相似度矩阵，只需要存储用户和物品的隐向量，空间复杂度由 $O(M^2)$或者$O(N^2)$降低到$O(M*k+N*k)$级别。
