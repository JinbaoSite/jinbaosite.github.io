...
$$
G_{i,j} = \mathrm{ReLU}\big(h(s_i)\big) \\
\qquad v_i = \sum_{j=1}^{N_e} G_{i,j}\, e_{i,j}(s_i)
$$
...
其中 $k$ 是调整 FFN 隐藏维度的缩放比例。而在 Sparse-MoE 版本中，每个 Token 的实际参数量和计算量会进一步被稀疏度 $s = \frac{\text{#}\mathrm{\text{Activated\_Param}}}{\text{#}\mathrm{\text{Total_Param}}}$ 缩放。
